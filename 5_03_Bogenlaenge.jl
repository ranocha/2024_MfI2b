...
let
	σ(x) = sqrt(1 - x^2)
	ℓ(x) = 3 * σ(x / 7)
	s(x) = 4.2 - 0.5 * x - 2.8 * σ(0.5 * x - 0.5)
	b(x) = σ(abs(2 - x) - 1) - x^2 / 11 + 0.5 * x - 3
	c(x) = [1.7, 1.7, 2.6, .9]
		...
		if bottom
			...
			if -1 < x < 1
				if -0.5 < x < 0.5
					y = 1.7
				elseif -0.8 < x < 0.8
					y = 1.7 + 0.9 * (xx - 0.5) / 0.3
				else
					y = 2.6 - 1.7 * (xx - 0.8) / 0.2
				end
			elseif -3 < x < 3
				y = s(xx)
			else
	...
	t = range(-14, 14, length = 5000)
	lines!(ax, γ.(t); label = L"\mathrm{im}(c)")

	t_polygon = range(first(t), last(t), length = n_batman)
	polygon = γ.(t_polygon)
	scatterlines!(ax, polygon;
				  label = "Polygon", color = Makie.wong_colors()[2])
...
c_1&\colon [-1, 1] \to \mathbb{R}^2, & c_1(t) &= \begin{pmatrix} t \\ t \end{pmatrix},
\\
c_2&\colon [-\pi / 2, 5 \pi / 2] \to \mathbb{R}^2, & c_2(t) &= \begin{pmatrix} \sin(t) \\ \sin(t) \end{pmatrix},
\end{aligned}\end{equation}$$

und durchlaufen deren Parameterbereich zum Anteil $p$.
"""

# ╔═╡ 293269e9-fd88-43a9-8a83-f69c13481f57
md"""
``p`` = $(@bind p_curve Slider(range(0, 1, length = 101), default=0.1, show_value=true))
"""

# ╔═╡ 3d2cf09b-85ed-4ffe-af27-40b35ea066f6
let p = p_curve
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"y")
	
	t = range(-1, 1, length = 100)
	lines!(ax, t, t; label = L"c_1")
	let
		t = t[begin + round(Int, p * (end - begin))]
		scatter!(ax, [t], [t]; label = L"$c_1$ (Anteil $p$)")
	end

	t = range(-0.5 * π, 2.5 * π; length = 500)
	lines!(ax, sin.(t), sin.(t); label = L"c_2", linestyle = :dot)
	let
		t = t[begin + round(Int, p * (end - begin))]
		scatter!(ax, [sin(t)], [sin(t)]; label = L"$c_2$ (Anteil $p$)")
	end
...
# ╔═╡ 58c112e6-2e95-4ec6-8a6f-53c1367c7986
md"""
## Die Funktion $x \mapsto x \sin(1 / x)$

Die Funktion ist stetig auf $\mathbb{R} \setminus \{0\}$ und stetig in $x = 0$ fortsetzbar mit Grenzwert $0$. Außer bei Null ist die Funktion auch stetig differenzierbar. Ihr Graph ist aber nicht das Bild einer rektifizierbaren Kurve,
denn für $x$ nahe Null kann man Punkte wählen, sodass sich die Länge eines
Polygonzugs wie die harmonische Reihe verhält.
"""

# ╔═╡ 983fff03-9eb3-4987-8ca7-4cb9125d243c
md"""
``x_\mathrm{max}`` = $(@bind xmax_x_sin_1_x Slider(10.0 .^ range(-2, 1, length = 101), default=0.1, show_value=true))
"""

# ╔═╡ 0b059e47-daf9-48b9-bf8d-ceddd3e14558
let
	fig = Figure()
