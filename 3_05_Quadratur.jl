...
md"""
# 3.5 Numerische Integration -- Quadratur
...
$$x_k = a + (k - 1) \Delta x, \quad k = 1, \dots, N,
\qquad
...
# ╔═╡ a041ab46-0167-4645-8e9a-3ce0fe725bc4
md"""
### Adaptive Quadratur

In der Praxis würde man eine adaptive Quadratur basieren auf einem Fehlerschätzer verwenden. Hier nutzen wir konkret die Gauß-Kronrod-Quadratur aus dem Julia-Paket [QuadGK.jl](https://juliamath.github.io/QuadGK.jl/stable/) für das Integral

$$\int_a^b \mathrm{e}^{-x^2} \,\mathrm{d}x.$$
"""

# ╔═╡ 0fea844e-d521-4e40-b466-34a6d7fa267d
md"""
``a`` = $(@bind a_adaptive Slider(range(-2, 2, step = 0.1), default = -2, show_value = true))

``b`` = $(@bind b_adaptive Slider(range(-2, 2, step = 0.1), default = 2, show_value = true))
"""

# ╔═╡ 41b248f8-3311-443d-b5e4-5455edd7fa24
let f = x -> exp(-x^2), a = a_adaptive, b = b_adaptive
	exact = sqrt(big(π)) * (erf(big(b)) - erf(big(a))) / 2
...
function trapezoidal(f, a, b, N)
	...
	result = (f(a) + f(b)) / 2
	result += sum(f, nodes[(begin + 1):(end - 1)])
	result *= step(nodes)
	return result
end
...
manifest_format = "2.0"
project_hash = "f77a76ee5990396cb0d42c2ea69609faad7782e9"
