...
ist eine raumfüllende Kurve. Solche raumfüllenden Kurven haben viele Anwendungen, etwa zur effiziente Iteration über Baum-basierte Datenstrukturen.
"""
...
let
	...
	# Function to plot Hilbert curve
	function plot_hilbert_curve(n::Int)
	    # Generate points for the n-th iteration
	    ...
	    ax = Axis(fig[1, 1], title = "Hilbert-Kurve (Iteration $n)", aspect = DataAspect())
	    lines!(ax, x, y)
		xlims!(ax, (-0.01, 1.01))
		ylims!(ax, (-0.01, 1.01))
	    fig
...
$$c\colon [0, 2\pi] \to \mathbb{R}^2, \quad c(t) = \begin{pmatrix} \cos(t) \\ \sin(t) \end{pmatrix}.$$

Der Geschwindigkeitsvektor ist

$$\dot{c}(t) = \begin{pmatrix} -\sin(t) \\ \cos(t) \end{pmatrix}$$

und der Beschleunigungsvektor ist

$$\ddot{c}(t) = \begin{pmatrix} -\cos(t) \\ -\sin(t) \end{pmatrix}.$$
"""
...
let
	...
	t = range(0.0, 2π, length = 500)
	lines!(ax, cos.(t), sin.(t); label = L"\mathrm{im}(c)")

	x = [cos(t_circle)]
	y = [sin(t_circle)]
	scatter!(ax, x, y; label = L"c(t)")
	
	u = [-sin(t_circle)]
	v = [cos(t_circle)]
	arrows!(ax, x, y, u, v; label = L"\dot{c}(t)",
			color = strokecolor = Makie.wong_colors()[2], linewidth = 2)
	
	u = [-cos(t_circle)]
	v = [-sin(t_circle)]
	arrows!(ax, x, y, u, v; label = L"\ddot{c}(t)",
			color = strokecolor = Makie.wong_colors()[3], linewidth = 2)
