...
Die lineare Advektionsgleichung/Transportgleichung ist

$$\begin{equation*}
\begin{aligned}
  \partial_t u(t, x) + \nabla \cdot \bigl(a(x) u(t, x) \bigr) &= 0,
  && t > 0,
  \\
  u(0, x) &= u^0(x).
\end{aligned}
...
Je nachdem, auf welchem räumlichen Gebiet die Transportgleichung gegeben ist,
muss sie noch mit passenden Randbedingungen ergänzt werden.
"""

# ╔═╡ a6bafae8-64e9-4192-92f3-7d91fce184cb
md"""
## Eindimensionaler Fall mit konstanter Geschwindigkeit
...
md"""
``t`` = $(@bind t_fourier_1 Slider(range(0, 10, length = 101), default = 0, show_value = true))
"""
...
# ╔═╡ 529aebe0-b180-4337-ae5d-c9136e7ba3ad
md"""
...
für $x \in [0, 1]^2 \subset \mathbb{R}^2$. Als erstes visualisieren wir die
Transportgeschwindigkeit.
"""

# ╔═╡ 14a725dd-a563-4fea-a107-de4229e99110
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"y", aspect = DataAspect())
	streamplot!(ax, xy -> Point2f(-xy[2], xy[1]),
				(-1, 1), (-1, 1))
	fig
end

# ╔═╡ ce09a2ee-7da3-4fd9-a894-93e1469015e4
let
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"y", aspect = DataAspect())
	x = range(-1.0, 1.0, length = 10)
	y = range(-1.0, 1.0, length = 10)
	arrows!(ax, x, y, xy -> 0.2 * Point2f(-xy[2], xy[1]))
	fig
end

# ╔═╡ 161c1fdd-f3bf-4367-9c5b-7b69fce21e41
md"""
Als nächstes visualisieren wir die Lösung zur Zeit $t$.
"""

# ╔═╡ f725bbbf-afa2-4a3b-a6c5-f0721ad74b38
md"""
``t`` = $(@bind t_2d_1 Slider(range(0, 10, length = 101), default = 0, show_value = true))
"""

# ╔═╡ d644f2a5-4dae-48ec-ba0c-2ba8616ed5d7
let t = t_2d_1
	fig = Figure()
	ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"y", aspect = DataAspect())
	x = range(-1.0, 1.0, length = 200)
	y = range(-1.0, 1.0, length = 200)
	function f0(x, y)
		sin(x) - exp(-20 * ((x - 0.5)^2 + y^2))
	end
	function f(x, y)
		r = sqrt(x^2 + y^2)
		φ = atan(y, x)
...
manifest_format = "2.0"
project_hash = "e83c19eab0c25b07b8a869d28536d955613ead03"
