...
md"""
# 3.1 Integral als orientierter Flächeninhalt

Wir haben das Integral als orientierten Flächeninhalt

$$\int_a^b f(x) \,\mathrm{d} x
  :=
  \mu(O_+(f)) - \mu(O_-(f))$$

eingeführt, wobei $\mu$ die Flächeninhalt bezeichnet und

$$\begin{equation*}
\begin{aligned}
  O_+(f) &:= \{ (x, y) \in \mathbb{R}^2 \mid a \le x \le b, 0 < y < f(x) \}, \\
  O_-(f) &:= \{ (x, y) \in \mathbb{R}^2 \mid a \le x \le b, f(x) < y < 0 \}
\end{aligned}
\end{equation*}$$

die Ordinatenmengen sind.
"""

# ╔═╡ 1672faa2-55b0-4099-948d-11c25d92dad1
@bind f_lebesgue Select([
	sinpi => "f(x) = sin(π x)",
	exp => "f(x) = exp(x)",
	(x -> exp(x) - 0.5) => "f(x) = exp(x) - 0.5"
])

# ╔═╡ 24571528-20e8-45ac-b618-c49a63521b65
md"""
``a`` = $(@bind a_lebesgue Slider(range(-2, 2, step = 0.1), default = -2, show_value = true))

``b`` = $(@bind b_lebesgue Slider(range(-2, 2, step = 0.1), default = 2, show_value = true))
"""

# ╔═╡ c2252554-d448-4d45-9a42-97ec34c79f32
let f = f_lebesgue, a = a_lebesgue, b = b_lebesgue
	fig = Figure()
...
# ╔═╡ 03222884-987c-402e-9d25-903862f8937d
md"""
## Riemann-Summen

Man kann das Integral $\int_a^b f(x) \, \mathrm{d}x$ von stetigen Funktionen $f$
...
manifest_format = "2.0"
project_hash = "b86984a2255b714a1b29fd03ad6f2e865551f6c0"
