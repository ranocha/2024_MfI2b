...
# ╔═╡ b0ef575e-8d54-4c61-887d-2549c31cd676
using TestImages, MosaicViews, ImageShow, ImageFiltering

# ╔═╡ e6c64c80-773b-11ef-2379-bf6609137e69
md"""
# 7.1 Diffusion

Die Diffusionsgleichung/Wärmeleitungsgleichung ist
...
Je nachdem, auf welchem räumlichen Gebiet die Difusionsgleichung gegeben ist,
...
Als erstes betrachten wir den eindimensionalen Fall
...
md"""
``t`` = $(@bind t_fourier_1 Slider(range(0, 1, length = 101), default = 0, show_value = true))
"""
...
let t = t_fourier_1
	...
	u = zeros(length(x), length(a))
	for i in eachindex(x)
		xi = x[i]
		for k in eachindex(a)
			u[i, k] = exp(-(k - 1)^2 * t) * a[k] * sin((k - 1) * xi)
		end
	end
...
# ╔═╡ 0219936b-e6f7-45eb-9c09-44e4dd57d738
md"""
Hier sehen wir das Verhalten für eine Linearkombination mehrerer Fourier-Moden.
Als erstes verschwinden die Anteile der höchsten Frequenzen, dann die im mittleren Bereich, und schließlich auch die niedrigen Frequenzen.
"""

# ╔═╡ 78c1be6e-fb70-4441-8bdc-24eb8857b9f0
md"""
``t`` = $(@bind t_fourier_2 Slider(range(0, 2, length = 101), default = 0, show_value = true))
"""

# ╔═╡ 141fc8eb-8348-454f-9bae-acd7773b63ee
let t = t_fourier_2
	# a = [
	# 	0.0
	# 	0.8287787225183392
...
# ╔═╡ 64836e98-f4ca-446f-a0f3-b9d14cf037d8
md"""
## Verknüpfung zum Filtern/Entrauschen von Bildern

Gaußsche Filter werden zum Filtern/Entrauschen von Bildern eingesetzt.
Konkret wird dabei eine Faltung mit einer Gaußschen Normalverteilung
der Standardabweichung $\sigma > 0$ vorgenommen. Das ist genau das
Verhalten der Wärmeleitungsgleichung zur Zeit $t = \sigma / 2$!

Die Gaußschen Filter werden häufig so konstruiert, dass bei einer Distanz von
$2 \sigma$ abgeschnitten wird. Die verbleibenden Werte werden ausgewertet und
normiert, sodass sie sich zu Eins aufsummieren.
"""

# ╔═╡ 46929fbf-f91c-494d-b864-7cdd01fd4196
md"""
``σ`` = $(@bind σ_plot1 Slider(0.5:0.1:10, default = 1, show_value = true))
"""

# ╔═╡ ed6d9252-5345-414d-b615-3770c1976342
let σ = σ_plot1
	k = Kernel.gaussian((σ,))
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel = L"x")
	scatter!(ax, k; label = "Filter")
	x = range(minimum(axes(k, 1)) - 1.2, maximum(axes(k, 1)) + 1.2, length = 200)
	lines!(ax, x, @. exp(-0.5 * (x / σ)^2) / (sqrt(2 * π) * σ); label = "Gauß-Verteilung")
	axislegend(ax, position = :lt)
	fig
end

# ╔═╡ 8e029daf-ebc0-4bac-8ab5-286d8ef4959c
md"""
Angewandt auf Bilder wie beispielsweise als Teil des 
Canny-Kantendetektionsalgorithmus, den wir früher besprochen haben,
verhalten sich Gaußsche Filter wie in den folgenden Beispielen.
"""

# ╔═╡ 8ed01fd0-b5c6-45ee-9be7-3b5a3da2e122
md"""
...
# ╔═╡ b50fcbfb-7d33-4ed5-8f85-7c91ebc9c527
md"""
``σ`` = $(@bind σ Slider(0.5:0.1:10, default = 1, show_value = true))
"""

# ╔═╡ 5a3c11b1-e0cd-4a2a-a454-581e794ff77b
begin
	# img_input = map(ColorTypes.Gray, testimage(img_name))
	img_input = testimage(img_name)
...
manifest_format = "2.0"
project_hash = "1f7d2e54095f24635d2a9fe15db2f78af38a4893"
