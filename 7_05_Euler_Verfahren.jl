...
md"""
# 7.5 Einfache Zeitintegrationsverfahren

Wir vergleichen das explizite und das implizite Euler-Verfahren für die ODE

$$u'(t) = - u(t), \quad u(0) = 1.$$
"""

# ╔═╡ c9861dd9-87af-4276-a93d-73c2f17532d4
md"""
``τ`` = $(@bind τ Slider(range(0.1, 10, length = 101), default = 0, show_value = true))
"""

# ╔═╡ 551d8388-6239-413c-925e-debb2f0a2e68
let
	t = range(0.0, max(10.0, 10 * τ), step = τ)
	u_explicit = ones(length(t))
	for i in eachindex(u_explicit)[begin+1:end]
		u_explicit[i] = (1 - τ) * u_explicit[i - 1]
	end
	u_implicit = ones(length(t))
	for i in eachindex(u_implicit)[begin+1:end]
		u_implicit[i] = u_implicit[i - 1] / (1 + τ)
	end

	fig = Figure()
	ax = Axis(fig[1, 1])
	scatterlines!(ax, t, u_explicit; label = "explizit")
	scatterlines!(ax, t, u_implicit; label = "implizit")
	t_ana = range(extrema(t)..., length = 200)
	u_ana = @. exp(-t_ana)
	lines!(ax, t_ana, u_ana; label = "exakt", color = :gray)
