...
# ╔═╡ ba800075-6d04-40f0-8c5d-749ef50bda77
...
In einer Raumdimension kann man
das Integral $\int_a^b f(x) \, \mathrm{d}x$ von stetigen Funktionen $f$
auf kompakten Intervallen $[a, b]$
auch als Grenzwert von *Riemann-Summen* der Form

$$\sum_{k=1}^N f(x_k) \Delta x_k$$

definieren, wobei die Stützstellen $x_k$ jeweils in Teilintervallen
von $[a, b]$ der Länge $\Delta x_k$ liegen. Die Riemannschen Obersummen
verwenden die Maximalstellen von $f$, die Untersummen die Minimalstellen von $f$.
Eine Funktion ist *Riemann-integrierbar*, wenn die Riemannschen Ober- und Untersummen bei einer Verfeinerung der Teilintervalle gegen den gleichen Wert konvergieren, der dann als Definition des Riemann-Integrals verwendet wird.
...
md"""
``N`` = $(@bind N_riemann Slider(2:100, default = 20, show_value = true))
...
let f = f_riemann, a = a_riemann, b = b_riemann, N = N_riemann
	...
	plt = band!(ax, x_sums, zero(f_lower), max.(f_lower, 0); 
			    color = Makie.wong_colors()[1], label = "Untersumme")
	translate!(plt, 0, 0, -2)
	...
	plt = band!(ax, x_sums, zero(f_upper), max.(f_upper, 0); 
			    color = Makie.wong_colors()[2], label = "Obersumme")
	translate!(plt, 0, 0, -5)
...
# ╔═╡ 74e3074f-2084-4163-b86f-ba83d078b62c
md"""
Wir können die Fläche unter dem Graphen von $f$ auch als zweidimensionalen
Bereich betrachten, dessen Volumen wir bestimmen sollen, indem wir über die
konstante Funktion $x \mapsto 1$ integrieren:

$$\int_a^b f(x) \, \mathrm{d} x = \int_Y 1 \, \mathrm{d} y,$$

wobei

$$Y = \{ (y_1, y_2)^T \in \mathbb{R}^2 \mid y_1 \in [a, b], y_2 \in [0, f(y_1)] \}.$$

Wenden wir hier die Verallgemeinerung des Riemannschen Zugangs auf das
Mehrdimensionale an, überdecken wir den Integrationsbereich $Y$ also durch
Rechtecke und lassen deren Flächeninhalt gegen Null gehen.
"""

# ╔═╡ b1f2223d-f7f3-463e-b9c6-8ac676755396
@bind f_riemann2 Select([
	exp => "f(x) = exp(x)",
])

# ╔═╡ 7312d4db-4d68-4369-80eb-24c514aaaa7a
md"""
``N`` = $(@bind N_riemann2 Slider(2:100, default = 20, show_value = true))

``a`` = $(@bind a_riemann2 Slider(range(-2, 2, step = 0.1), default = -2, show_value = true))

``b`` = $(@bind b_riemann2 Slider(range(-2, 2, step = 0.1), default = 2, show_value = true))
"""

# ╔═╡ c9531af4-facb-4c12-9254-a0b9cab4dc07
let f = f_riemann2, a = a_riemann2, b = b_riemann2, N = N_riemann2
	fig = Figure()
	...
	x = range(-2.1, 2.1, length = 10^4)
	f_x = @. f(x)
	lines!(ax, x, f_x; color = :gray, linestyle = :dot)
	
	x = range(a, b, length = 10^4)
	f_x = @. f(x)
	lines!(ax, x, f_x; color = :black)
	lines!(ax, [a, a], [0, f(a)]; color = :black)
	lines!(ax, [b, b], [0, f(b)]; color = :black)
	lines!(ax, [a, b], [0, 0]; color = :black)

	# Riemann quads
	x_boundaries = range(a, b; length = N + 1)
	h = step(x_boundaries)

	quads = [Rect(a, b, 1.0, 1.0)]
	empty!(quads)
	
	for k in 1:N
		xmin = x_boundaries[k]
		xmax = x_boundaries[k + 1]
		y = range(xmin, xmax, length = 10^2)
		fmin = minimum(f, y)

		ymin = 0.0
		ymax = h
		while fmin >= ymax
			push!(quads, Rect(xmin, ymin, h, h))
			ymin = ymax
			ymax += h
		end
	end
	
	poly!(ax, quads; color = eachindex(quads),
...
# ╔═╡ cbccb3f5-0cb2-4fd5-b938-c66d0f194926
md"""
Das ist bisher nur eine kompliziertere Variante der 1D-Riemannschen (Unter-) Summe.
Wir können damit aber auch nicht-konstante Funktionen über mehrdimensionale
Bereiche integrieren wie im folgenden Beispiel.
"""

# ╔═╡ 8b2b613e-7111-421d-851d-6d6d5a34d0e4
@bind f_riemann3 Select([
	(x -> sum(abs2, x)) => "f(x) = x₁² + x₂²",
])

# ╔═╡ 85fed3f6-8316-4787-878e-747a121c6e68
md"""
``N`` = $(@bind N_riemann3 Slider(2:100, default = 10, show_value = true))
"""

# ╔═╡ 1f26c852-985c-4324-a819-c7fff873324e
let f = f_riemann3, N = N_riemann3
	fig = Figure()
	ax = Axis3(fig[1, 1]; xlabel = L"x_1", ylabel = L"x_2", zlabel = L"f(x)")

	# function
	x = range(0.0, 1.0, length = 100)
	y = range(0.0, 1.0, length = 100)
	z = [f((x, y)) for x in x, y in y]
	# surface!(ax, x, y, z; alpha = 0.5)

	# hexaeder
	x_boundaries = range(extrema(x)..., length = N + 1)
	y_boundaries = range(extrema(y)..., length = N + 1)

	hexes = [Rect(x[1], y[1], 0.0, 1.0, 1.0, 1.0)]
	empty!(hexes)
	...
	for i in 1:N, j in 1:N
		xmin = x_boundaries[i]
		xmax = x_boundaries[i + 1]
		ymin = y_boundaries[j]
		ymax = y_boundaries[j + 1]
		zmin = 0.0
		zmax = f(((xmin + xmax) / 2, (ymin + ymax) / 2))
		push!(hexes, Rect(xmin, ymin, zmin,
						  step(x_boundaries), step(y_boundaries), zmax))
		push!(heights, zmax)
	end

	poly!(ax, hexes; color = heights)
	# surface!(ax, x, y, z; alpha = α)
