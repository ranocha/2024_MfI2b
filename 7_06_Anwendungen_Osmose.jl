...
using TestImages, MosaicViews, ImageEdgeDetection, Images
...
md"""
# 7.6 Anwendungen der Osmose in der Bildverarbeitung


"""

# ╔═╡ 6080d1b7-dc0e-4d2e-bf68-b3ef6516f963
md"""
...
- Joseph-Louis Lagrange
  von [http://www.hrono.ru/biograf/bio_l/lagranzhzh.html](https://commons.wikimedia.org/wiki/File:%D0%9B%D0%B0%D0%B3%D1%80%D0%B0%D0%BD%D0%B6.jpg), Public domain, via Wikimedia Commons
- Leonhard Euler von 
...
begin
	img_lagrange_original = load(download("https://upload.wikimedia.org/wikipedia/commons/4/46/%D0%9B%D0%B0%D0%B3%D1%80%D0%B0%D0%BD%D0%B6.jpg"))
	img_euler_original = load(download("https://upload.wikimedia.org/wikipedia/commons/d/d7/Leonhard_Euler.jpg"))

	img_lagrange_original_cropped = map(RGB{Float64}, 
		img_lagrange_original)
	...
	mosaicview(img_lagrange_original_cropped, img_euler_original_cropped; nrow = 1)
end
...
md"""
Stattdessen nehmen wir die entsprechenden Anteile der (diskreten) kanonischen
Transportgeschwindigkeiten beider Bilder und lösen damit die lineare Osmose-Gleichung
numerisch bis zum stationären Zustand (mit einer gewissen Toleranz).
"""
...
# ╔═╡ d072dbe6-d8a3-4125-acc6-5e37ab56ef8a
md"""
Als nächstes nehmen wir zwei Bilder von US-Präsidenten als Ausgangspunkt.

- George Washington
  von [Gilbert Stuart](https://commons.wikimedia.org/wiki/File:Gilbert_Stuart_Williamstown_Portrait_of_George_Washington.jpg), Public domain, via Wikimedia Commons
- Abraham Lincoln
  von [George Henry Story](https://commons.wikimedia.org/wiki/File:Lincoln_by_George_H_Story_c1915.jpg), Public domain, via Wikimedia Commons
"""

# ╔═╡ 7e9828df-6197-4da9-8d59-2e8ce6dda806
begin
	img_washington_original = load(download("https://upload.wikimedia.org/wikipedia/commons/b/b6/Gilbert_Stuart_Williamstown_Portrait_of_George_Washington.jpg"))
	img_lincoln_original = load(download("https://upload.wikimedia.org/wikipedia/commons/7/78/Lincoln_by_George_H_Story_c1915.jpg"))

	mosaicview(img_washington_original, img_lincoln_original, nrow = 1)
end

# ╔═╡ 7e42cdae-3f45-4429-b7b1-e40038673d31
md"""
Die Bilder haben verschiedene Größen und müssen daher skaliert werden.
Außerdem schauen die beiden Präsidenten in umgekehrte Richtungen, wir müssen
ein Bild also spiegeln. Wir konzentrieren uns außerdem nur auf die Gesichter.
"""

# ╔═╡ 69f57d51-10c5-4f4d-9742-fb09a26a51f0
begin
	img_washington_original_cropped = map(RGB{Float64}, imresize(
		img_washington_original[651:3750, 751:3850],
		(256, 256)))
	img_lincoln_original_cropped = map(RGB{Float64}, imresize(
		img_lincoln_original[301:2000, 201:1900],
		(256, 256))[:, end:-1:begin])
	mosaicview(img_washington_original_cropped, img_lincoln_original_cropped, nrow = 1)
end

# ╔═╡ c953da2a-a3e5-41d9-82f7-14096ddf5e65
md"""
Eine einfache Kopie des Gesichtes ist natürlich wieder stümperhaft.
"""

# ╔═╡ a79cde42-bb80-4b45-8f7a-7d4e6801dda0
begin
	idx_us = BitMatrix(open(joinpath(@__DIR__, "lincoln_indices.txt"), "r") do io
		readdlm(io)
	end)
	@. img_washington_original_cropped * !idx_us + img_lincoln_original_cropped * idx_us
end

# ╔═╡ 2c1ff047-6e73-4692-8521-4d6779b1394a
md"""
Wenn wir stattdessen wieder die entsprechenden Anteile der (diskreten) kanonischen
Transportgeschwindigkeiten beider Bilder nehmen und damit die lineare 
Osmose-Gleichung numerisch bis zum stationären Zustand (mit einer gewissen Toleranz)
lösen, ist das Ergebnis besser.
"""

# ╔═╡ 31658320-4cc4-4ba7-9c3d-659ed9b259ef
md"""
Die Farbgebung des Endresultats hängt davon ab, mit welchem Bild wir als
Anfangszustand starten, da der mitlere Wert aller Farbkanäle erhalten bleibt.
"""
...
function merge_images(img1, idx1, img2, idx2; kwargs...)
	img = map(red, img1)
	...
	img_red = run_osmosis(img, d; kwargs...)
	...
	img_green = run_osmosis(img, d; kwargs...)
	...
	img_blue = run_osmosis(img, d; kwargs...)
...
# ╔═╡ e30d7acb-92eb-49e5-83f2-f2fcb9b85188
let
	img1 = merge_images(
		img_lincoln_original_cropped, idx_us,
		img_washington_original_cropped, .!(idx_us))
...
manifest_format = "2.0"
project_hash = "c184bec607ed2865412fea31b4be2efb8ad1303a"
