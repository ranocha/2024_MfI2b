...
$$f\colon [-1, 1] \to \mathbb{R}, \qquad f(x) = \mathrm{e}^{\sin(\pi x)}0,$$
...
md"""
``n`` = $(@bind n_expsinpi Select(0:30, default=0))
"""
...
md"""
``n`` = $(@bind n_cos0 Select(0:30, default=0))
"""
...
md"""
``n`` = $(@bind n_sign Select(0:30, default=0))
"""

# ╔═╡ a161897c-a8aa-4b21-9be7-46aeec3623b0
md"""
Hier können wir ebenfalls einige typische Verhaltensweisen beobachten:
- Ausreichend weit von der Unstatigkeitsstelle entfernt scheint die Approximation weiterhin punktweise zu konvergieren - allerdings noch langsamer als vorher.
