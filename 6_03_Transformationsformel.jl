...
Sei $\psi\colon \mathbb{R}^n \supset Y \to X \subset \mathbb{R}^n$ eine stetig 
differenzierbare Koordinatentransformation, d.h., $\psi$ ist bijektiv und fast 
überall gilt $\det \psi' \neq 0$.

Nach der Transformationsformel gilt dann für stetiges
$f\colon \mathbb{R}^n \supset \psi(Y) \to \mathbb{R}$

$$\int_{\psi(Y)} f(x) \,\mathrm{d} x = \int_Y f(\psi(y)) \left| \det \psi'(y) \right| \,\mathrm{d} y.$$

Hier betrachten wir einen Kreisbogen und Polarkoordinaten im $\mathbb{R}^2$.
"""
